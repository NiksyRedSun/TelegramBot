            ...
            if damage <= 0:
                ...
                if not char.alive:
                    if char.quoteIndex is not None:
                        ...
                        await message.answer(text=self.killed_quotes[self.quoteIndex], reply_markup=next_keyb)
        else:
            await message.answer(text=self.pos_quotes[self.quoteIndex])
        self.quoteIndex = None
...
class SceletonMob(Mob):
    def __init__(self):
        ...
        self.killed_quotes = [f"Скелет просто уходит оставляя вас умирать, оставив свой меч в ваших ребрах на прощание",
...
class LittleDragonMob(Mob):
    def __init__(self):
        ...
        self.killed_quotes = [f"Падая на колени вы чувствуйте сильный укус на своем теле, разрывая рубашку вы видите следы зубов дракона,"
...
class OrcMob(Mob):
    def __init__(self):
        ...
        self.killed_quotes = [f"Мгновение назад, вы отвлеклись на боль от ранения. А теперь "
                             f"вы чувствуете как вам в шею сзади помещается кинжал. Спазм шейных мышц заставляет вас еще "
                              f"раз посмотреть на небо перед смертью",
                            f"Последнее, что вы видите, падая на землю, это пятку забивающего вас до смерти Орка",
                             f"После небольшой потери равновесия вы теряете врага из виду, и находите кинжал у себя в пояснице."
                            f" Отнявшиеся ноги не позволяют вам двигаться"
