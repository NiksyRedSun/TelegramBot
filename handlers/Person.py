...
async def person_menu(message: types.Message, state: FSMContext):
    ...
    if message.text == "Изменить имя":
        ...
        await message.answer(text="Здесь можно распределить или перераспределить очки умений.\n"
                                  "Параметр \"Атака\" отвечает за количество урона, наносимое противнику при попадании\n"
                                  "Параметр \"Защита\" отвечает за количество урона, которое будет поглащено при попадании противником\n"
                                  "Параметр \"Ловкость\" отвечает за вероятность попасть по противнику или увернуться\n"
                                  f"Очков умений у вас: {char.points}\n"
                                  "Увеличиать атаку - 1 очко\n"
                                  "Увеличить защиту - 1 очко\n"
                                  "Увеличить ловкость - 3 очка\n"
